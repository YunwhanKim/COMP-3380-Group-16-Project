...
class MyDatabase {
    ...
    public MyDatabase() {
        try {
            ...
            // NHL_DATA_V1_5.db 파일 경로 (실행 위치에 db 파일이 있다고 가정)
            String url = "jdbc:sqlite:NHL_DATA_V1.5.db";

            this.connection = DriverManager.getConnection(url);

            if (this.connection != null) {
                System.out.println("✅ Successfully connected to NHL_DATA_V1.5.db!");
            }

        } catch (Exception e) {
            System.out.println("Connection failed!");
            System.out.println("Checklist: 1. NHL_DATA_V1.5.db in current directory? 2. sqlite-jdbc driver on classpath?");
            e.printStackTrace();
        }
    }
    ...
    // -----------------------------------------------------------------------
    public void query1() {
        // SQLite: CAST(... AS UNSIGNED) 미지원 → CAST(ss.gameID / 1000000 AS INTEGER) 사용
        String sql =
            ...
            "WHERE CAST(ss.gameID / 1000000 AS INTEGER) = 2019 " +
